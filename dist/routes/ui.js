...
(function(){
  ...
  function renderBomTab(scanReports){
    ...
    var capInfo={network:["Connects to the internet","This skill sends or receives data over the network.","\u{1F310}"],file_access:["Reads or writes files","This skill accesses files on your system.","\u{1F4C1}"],env_read:["Reads environment variables","This skill checks settings like API keys from your environment.","\u{1F511}"],subprocess:["Runs commands","This skill executes shell commands or external programs.","\u2699\uFE0F"]};
  ...
  function renderFullBom(bom){
    ...
    if(fileEntries.length>0){
      html+='<div class="bom-section card"><h4 class="bom-section-title">\u{1F4C1} Files this skill touches</h4><p class="bom-section-desc">Files the skill reads from, writes to, or deletes.</p><div class="bom-table-wrap"><table class="bom-table"><thead><tr><th>Location</th><th>Action</th><th>Target file</th></tr></thead><tbody>';
    ...
    if((net.urls||[]).length>0){
      html+='<div class="bom-section card"><h4 class="bom-section-title">\u{1F310} Websites and APIs this skill contacts</h4><p class="bom-section-desc">External servers this skill communicates with.</p>';
    ...
    if((env.env_vars||[]).length>0||(env.binaries||[]).length>0){
      html+='<div class="bom-section card"><h4 class="bom-section-title">\u{1F511} Settings and tools this skill uses</h4><p class="bom-section-desc">Environment variables (like API keys) and command-line tools the skill relies on.</p>';
    ...
    if((df.inputs||[]).length>0||(df.outputs||[]).length>0){
      html+='<div class="bom-section card"><h4 class="bom-section-title">\u{1F504} How data moves through this skill</h4><p class="bom-section-desc">Where the skill gets data from and where it sends data to.</p><div class="bom-dataflow">';
    ...
    if(hasDeps2){
      html+='<div class="bom-section card"><h4 class="bom-section-title">\u{1F4E6} Libraries and tools referenced</h4><p class="bom-section-desc">Code libraries and shell tools mentioned in the skill\\'s files.</p>';
  ...
  function buildArenaCard(arena){
    ...
    card.innerHTML='<div class="arena-row-icon"><svg aria-hidden="true" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="3" width="8" height="18" rx="1"/><rect x="13" y="3" width="8" height="18" rx="1"/></svg></div>'
      ...
      +'<div class="arena-row-meta"><span>'+entries+' '+(entries===1?"entry":"entries")+'</span><span class="meta-dot">\xB7</span><span>'+formatDate(arena.created_at)+'</span></div></div>'
